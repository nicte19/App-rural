...
/* =========================================================
   PLACEHOLDERS MÍNIMOS PARA SECCIONES EN CONSTRUCCIÓN
========================================================= */
function renderProcedureProducerSelect() {
  const medSelect = $("#p_medSelect");
  const vaccineSelect = $("#p_vaccineSelect");
  const supplySelect = $("#p_supplySelect");

  if (medSelect) {
    medSelect.innerHTML = ['<option value="">— Selecciona —</option>']
      .concat(state.meds.map(item => `<option value="${escapeHtml(item.id || "")}">${escapeHtml(item.name || item.product || "Medicamento")}</option>`))
      .join("");
  }
  ...
  if (vaccineSelect) {
    const vaccines = [];
    state.producers.forEach(prod => {
      ensureQuestionnaire(prod).vaccines?.forEach(v => vaccines.push(v));
    });

    vaccineSelect.innerHTML = ['<option value="">— Selecciona —</option>']
      .concat(vaccines.map((item, index) => `<option value="${escapeHtml(item.id || String(index))}">${escapeHtml(item.name || item.product || item.biologic || "Vacuna")}</option>`))
      .join("");
  }

  if (supplySelect) {
    supplySelect.innerHTML = ['<option value="">— Selecciona —</option>']
      .concat(state.supplies.map(item => `<option value="${escapeHtml(item.id || "")}">${escapeHtml(item.name || item.product || "Insumo")}</option>`))
      .join("");
  }
}
